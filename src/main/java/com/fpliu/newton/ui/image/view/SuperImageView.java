...
public class SuperImageView extends AppCompatImageView {
    ...
    private static final int NONE = 0;// init state
    private static final int DRAG = 1;// drag
    private static final int ZOOM = 2;// zoom
    private static final int ROTATE = 3;// rotate
    private static final int ZOOM_OR_ROTATE = 4; // zoom or rotate
    ...
    private void initImage() {
        ...
        maxScale *= 2.25;//TODO calculate the max scale
        setImageMatrix(matrix);
    }
    ...
    private float maxPostScale() {
        ...
        float maxScale = Math.max(minScale, this.maxScale);
    ...
    private void doubleClick(float x, float y) {
        ...
        float minScale = Math.max((viewW - leftMargins * 2) / rotatedImageW, (viewH - topBottomMargins) / rotatedImageH);
        if (curScale <= minScale + 0.01) { // zoom
            float toScale = Math.max(minScale, maxScale) / curScale;
